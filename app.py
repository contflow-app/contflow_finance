...
if uploaded_file:
    ...
    # Listas suspensas
    categorias = plano['Categoria'].unique().tolist()
    subcategorias = plano['Subcategoria'].unique().tolist()
...
# Interface para Classificação Manual em tabela
...
if not nao_classificados.empty:
    st.write("Lançamentos Não Classificados")
    for idx, row in nao_classificados.iterrows():
        st.write(f"Data: {row['Data']}, Valor: {row['Valor']}, Descrição: {row['Descricao']}")
        tipo = st.selectbox(f"Tipo para {row['Descricao']}", ["Entrada", "Saída"], key=f"tipo_{row['rowid']}")
        categoria = st.selectbox(f"Categoria para {row['Descricao']}", categorias, key=f"categoria_{row['rowid']}")
        subcategoria = st.selectbox(f"Subcategoria para {row['Descricao']}", subcategorias, key=f"subcategoria_{row['rowid']}")
