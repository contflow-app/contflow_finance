...
if not nao_classificados.empty:
    for idx, row in nao_classificados.iterrows():
        ...
        tipo = st.selectbox(f"Selecione o Tipo para {row['Descricao']}", ["Entrada", "Saída"], key=f"tipo_{idx}")
        categoria = st.text_input(f"Categoria para {row['Descricao']}", key=f"categoria_{idx}")
        subcategoria = st.text_input(f"Subcategoria para {row['Descricao']}", key=f"subcategoria_{idx}")
        
        if st.button(f"Salvar {row['Descricao']}"):
            c.execute("""
                UPDATE lancamentos 
            ...
            conn.commit()
            st.success(f"Lançamento {row['Descricao']} atualizado com sucesso!")
else:
